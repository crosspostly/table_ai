...
function saveConfigAsPreset(presetName, config) {
  try {
    ...
    let presetsSheet = ss.getSheetByName('ConfigPresets');
    ...
    if (!presetsSheet) {
      ...
      // Создаем заголовки
      const headers = ['PresetName', 'SystemPromptSheet', 'SystemPromptCell', 'UserDataJSON', 'CreatedAt', 'LastUsed', 'Description'];
    ...
    const existingRow = findPresetRow(presetsSheet, presetName);

    // Подготавливаем данные
    const currentTime = new Date().toISOString();
    const systemPromptSheet = config.systemPrompt ? config.systemPrompt.sheet : '';
    const systemPromptCell = config.systemPrompt ? config.systemPrompt.cell : '';
    const userDataJSON = JSON.stringify(config.userData || []);
    ...
    if (existingRow > 0) {
      ...
      const lastRow = presetsSheet.getLastRow();
      var rowData = [
        [presetName, systemPromptSheet, systemPromptCell, userDataJSON,
...
function loadConfigPreset(presetName) {
  try {
    ...
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const presetsSheet = ss.getSheetByName('ConfigPresets');
    ...
    const rowIndex = findPresetRow(presetsSheet, presetName);
    ...
    const rowData = presetsSheet.getRange(rowIndex, 1, 1, 7).getValues()[0];
...
function getAllConfigPresets() {
  try {
    ...
    const data = presetsSheet.getRange(2, 1, lastRow - 1, 7).getValues();
    const presets = [];
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      try {
        if (row[3]) {
          const userData = JSON.parse(row[3]);
...
function applyPresetToCell(presetName, targetSheetName, targetCellAddress) {
  try {
    ...
    // Загружаем пресет
    const preset = loadConfigPreset(presetName);
    ...
    const saveResult = saveCollectConfig(targetSheetName, targetCellAddress, config);
    ...
    // Выполняем запрос
    const executeResult = executeCollectConfig(targetSheetName, targetCellAddress);
...
function findPresetRow(presetsSheet, presetName) {
  ...
  const data = presetsSheet.getRange(2, 1, lastRow - 1, 1).getValues();
...
function showPresetsManager() {
  try {
    ...
    let message = '📋 ВАШИ ПРЕСЕТЫ КОНФИГУРАЦИЙ\\n\\n';

    for (let i = 0; i < Math.min(presets.length, 10); i++) {
      const preset = presets[i];
      const createdDate = preset.createdAt ? new Date(preset.createdAt).toLocaleString('ru-RU') : 'Неизвестно';
      const lastUsedDate = preset.lastUsed ? new Date(preset.lastUsed).toLocaleString('ru-RU') : 'Никогда';
...
function applyPresetToCurrentCell() {
  try {
    ...
    const targetCellAddress = range.getA1Notation();
    ...
    const selectedPresetName = response.getResponseText().trim();
    ...
    const result = applyPresetToCell(selectedPresetName, targetSheetName, targetCellAddress);
...
function exportPresetsToJSON() {
  try {
    ...
    for (let i = 0; i < presets.length; i++) {
      const preset = loadConfigPreset(presets[i].name);
      if (preset) {

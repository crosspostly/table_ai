...
### Gemini API (два уровня приоритета)

```
ПРИОРИТЕТ:
1️⃣ ЛИЧНЫЙ КЛЮЧ (в свойствах клиента)
   └─ Если установлен → используется ЭТОт ключ
   
2️⃣ СЕРВЕРНЫЙ КЛЮЧ (в свойствах сервера)
   └─ Если личного нет → используется ЭТОТ ключ

...
function extractSourcesV2_(textVal, formula, richUrl) {
  ...
  try {
    ...
    (cleaned.match(
      /(?:^|\s)(?:vk\.com|drive\.google\.com|docs\.google\.com|yadi\.sk|disk\.yandex\.(?:ru|com)|dropbox\.com|script\.google\.com|script\.googleusercontent\.com)\/[^\s<>\)\]"]+/gi
    ) || []),
      .forEach(function(s) {
        push(String(s).trim());
      });
...
function gmOcrFromBlobV2_(blob, lang) {
  // Используем сервер как прокси для Gemini API (правильная аутентификация)
  ...
  if (code !== 200 || !data || !data.ok) {
    const msg = (data && data.error) || ('HTTP_' + code);
...
function serverGmOcrBatchV2_(images, lang) {
  const email = (typeof getLicenseEmail === 'function') ? getLicenseEmail() : '';
  const token = (typeof getLicenseToken === 'function') ? getLicenseToken() : '';
  const payload = {action: 'gm_image', email: email, token: token, images: images, lang: lang || 'ru', delimiter: '____'};

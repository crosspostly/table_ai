# 🤖 Table AI - Multifunctional Google Sheets Assistant

## 📋 ОГЛАВЛЕНИЕ
1. [Что это](#что-это)
2. [AI Constructor v3.0.0](#ai-constructor-v300)
3. [Export to Word/PDF](#export-to-wordpdf)
4. [Архитектура](#архитектура)
5. [Файлы проекта](#файлы-проекта)
6. [API: Клиент → Сервер](#api-клиент--сервер)
7. [Развертывание](#развертывание)
8. [Как работает](#как-работает)
...
**Table AI** — многофункциональный ассистент для Google Sheets с двумя основными модулями:

### 🤖 AI Constructor v3.0.0
Система для сбора данных из Google Sheets и отправки их в Gemini AI.

**Проблема:** Google Sheets ограничивает формулы 50,000 символами.
```javascript
=GM("Промпт: " & A1 & A2 & ... & A1000)
```
❌ Формула слишком длинная = ОШИБКА!

**Решение:** AI Конструктор собирает данные **на сервере** Apps Script:
...
✅ Сохранение конфигураций в ConfigData
✅ Шаблоны для повторного использования

### 📄 Export to Word/PDF
Отдельный модуль для экспорта листов Google Sheets в документы Word и PDF.

**Особенности:**
- Экспорт больших таблиц (батчовая обработка)
- Текстовые карточки вместо таблиц для лучшей читаемости
- OAuth авторизация (Drive + Docs)
...
### Хранение данных
Вся конфигурация хранится в скрытом листе **ConfigData**:

| Колонка | Назначение |
|---------|-----------|
| SheetName | Название листа (для конфигураций ячеек) |
| CellAddress | Адрес ячейки (для конфигураций ячеек) |
| SystemPromptSheet | Лист с system prompt |
| SystemPromptCell | Ячейка с system prompt |
| UserDataJSON | JSON массив источников данных |
| TemplateName | Название шаблона (пусто для конфигураций) |
| IsTemplate | true/false (отличает шаблоны от конфигураций) |
| CreatedAt | Дата создания |
| LastRun | Дата последнего запуска |

### Шаблоны
Шаблоны хранятся в том же листе ConfigData с флагом IsTemplate=true.

**Важно:** НЕ используется PropertiesService!
...
## 📄 Export to Word/PDF (отдельный модуль)

### Назначение
Экспорт листов Google Sheets в документы Word (.docx) и PDF.

### Меню
📄 Экспорт → 📄 Экспорт в Word/PDF

### Файлы
- `ExportToDocument.gs` - серверная логика
- `ExportToDocumentUI.html` - UI sidebar

### Стратегии обработки
- **FULL** (≤100 строк): полный экспорт, ~20-30 сек
- **BATCHES** (101-500): батчи по 50 строк, ~2-4 мин  
- **LIMITED** (>500): только первые 100 строк + warning

### Формат экспорта
Текстовые карточки (не таблицы):
```
📌 Запись #1
Поле1: Значение1
Поле2: Значение2
...
- Google Drive (создание файлов)
- Google Docs (создание документов)

### Сохранение
Файлы сохраняются в папку "Table AI Exports" на Google Drive.
...
|------|-------|-----------|
| `CollectConfig.gs` | 760+ | **AI Constructor** - серверная логика, ConfigData, шаблоны |
| `CollectConfigUi.html` | 983 | **AI Constructor UI** - интерфейс конструктора |
| `Main.gs` | 1350+ | **Основной модуль** - меню, GM(), VK Import, OCR |
| `ExportToDocument.gs` | 564 | **Export модуль** - экспорт в Word/PDF |
| `ExportToDocumentUI.html` | 261 | **Export UI** - sidebar для экспорта |
| `TemplateService.gs` | 20 | **DEPRECATED** - больше не используется |
| `SettingsUI.html` | 9981 | **Настройки** - единый UI настроек |
| `server.gs` | 293 | **Legacy меню** - дополнительные функции |

# 🎯 TEMPLATE SYSTEM v2.0 - Документация
...
- [Преимущества v2.0](#преимущества-v20)
...
- 🗂 **Локальное хранение** — ScriptProperties в проекте таблицы (без внешнего сервера)
- ⚡ **Быстрота** — PropertiesService (в 10× быстрее листов)
...
| Характеристика | v1.0 (ConfigData) | v2.0 (Template System) | Улучшение |
|----------------|-------------------|------------------------|-----------|
| **Хранение** | Лист ConfigData | ScriptProperties (локально) | 10x быстрее |
| **Переиспользование** | 1 конфиг → 1 ячейка | 1 шаблон → ∞ ячеек | ∞ раз |
| **Namespace** | Общий лист | "default" (без email) | Консистентно |
...
│   └── Per-template keys (namespace 'default')
└─────────────────────────────────────────┘
```

### Хранение данных:

**Per-template в ScriptProperties:**

- Префикс ключа: `COLLECT_TPL_V2:`
- Формат ключа: `COLLECT_TPL_V2:default:<TemplateName>`
- Значение: JSON `{ "config": {..}, "created": ISO, "updated": ISO, "version": "1.0" }`

Пример ключей:

```
COLLECT_TPL_V2:default:По умолчанию → {config, created, updated, version}
COLLECT_TPL_V2:default:Анализ отзыва → {config, created, updated, version}
```
...
### Автомиграция (v2.0 → v2.1)

- При первом обращении к шаблонам выполняется автоматическая миграция из старого монолитного ключа `COLLECT_CONFIG_TEMPLATES` в пер-шаблонные ключи `COLLECT_TPL_V2:*`.
- Если ранее использовались email‑неймспейсы, все данные объединяются в namespace `default` с безопасным разрешением конфликтов имён.
...
1. Имя с опечаткой
2. Шаблон удалён
3. Шаблон сохранён в другом проекте/таблице

...
### Utils
File: `shared/Utils.gs`
- `convertMarkdownToReadableText(markdownText: string): string`
- `processGeminiResponse(response: string): string`
- `isMarkdownText(text: string): boolean`
- `addSystemLog(message, level?, category?): void`
- `getSystemLogs(limit?, level?, category?): string`
- `exportSystemLogsToSheet(): void`
- `clearSystemLogs(): void`
- `safeJsonParse(jsonString, defaultValue?): any`
- `safeJsonStringify(obj, defaultValue?): string`
- `isValidEmail(email: string): boolean`
- `generateTraceId(prefix?): string`
- `sleep(milliseconds): void`
- `getNestedProperty(obj, path, defaultValue?): any`
- `formatFileSize(bytes: number): string`
- `logMessage(message, level?): void` (alias for `addSystemLog`)
- `truncateString(str, maxLength): string`
- `escapeHtml(text: string): string`
- `logServer(message, traceId?): void`
- `logClient(message): void`
- `extractImageSources(cellData, cellFormula?, richTextUrl?): ReturnType<extractSources>`
- `createDebounce(func, wait): (...args) => any` (throttling wrapper via CacheService)

### LoggingService
File: `shared/LoggingService.gs`
- `logToSheet(logEntry, level?): void`
- `generateTraceId(prefix?): string`
- `maskEmail(email: string): string`
- `logLicenseActivity(action, email, token, result, traceId?): void`
- `logSecurityEvent(event, context): void`
- `addSystemLogThrottled(message, level, component, throttleMs): void`
- `addSystemLogsBulk(entries: Array, async?: boolean): void`
- `getSystemLogs(level?, component?, limit?): Array`
- `cleanupOldLogs(daysToKeep): void`
- `getLoggingStats(): {total, byLevel, byComponent, ...}`

### DetailedLogger
File: `shared/DetailedLogger.gs`
- `initLogsSheet(): GoogleAppsScript.Spreadsheet.Sheet`
- `logToSheet(type, functionName, operation, status, details?, error?, duration?): void`
- `OperationLogger(type, functionName, operation): { start, success, error }`
- `clearOldLogs(keepLast): void`
- `exportLogsAsText(limit): string`
- `showRecentLogs(): void`
- `exportAndShowLogs(): void`
- `clearOldLogsUI(): void`

### EmojiRemover
File: `shared/EmojiRemover.gs`
- `removeEmojis(text: string): string`
- `containsEmojis(text: string): boolean`
- `countEmojis(text: string): number`

### VersionInfo
File: `shared/VersionInfo.gs`
- `getCurrentVersion(): string`
- `getLastUpdateDate(): string`
- `getVersionInfo(): {version, lastUpdate, build?}`
- `getVersionWithTimestamp(): string`
- `showVersionInfo(): void`

### SecurityValidator
File: `shared/SecurityValidator.gs`
- Types
  - `SecurityValidator.ValidationTypes` — `EMAIL | API_KEY | PROMPT | URL | GENERAL`
  - `SecurityValidator.ErrorTypes` — standard error categories
- Validators
  - `SecurityValidator.validateInput(input, type): {isValid, sanitized, errors}`
  - `SecurityValidator.validateEmail(email): {isValid, sanitized, errors}`
  - `SecurityValidator.validateApiKey(apiKey): {isValid, sanitized, errors}`
  - `SecurityValidator.validatePrompt(prompt): {isValid, sanitized, errors}`
  - `SecurityValidator.validateUrl(url): {isValid, sanitized, errors}`
  - `SecurityValidator.validateGeneral(input): {isValid, sanitized, errors}`
  - `SecurityValidator.validateGMParams(maxTokens?, temperature?): {isValid, sanitized:{maxTokens,temperature}, errors}`
- Logging & safety
  - `SecurityValidator.safeLog(message, data): void`
  - `SecurityValidator.sanitizeForLogging(data: string|object): string`
  - `SecurityValidator.testCredentialBoundaries(): Array<{test, passed, details?}>`
- Error handling
  - `handleSecureError(error, context?): string` — safe, user-facing error messages with sensitive data masked.
